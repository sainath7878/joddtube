...
export const removeAllNotesForVideoHandler = function (schema, request) {
    ...
    if (user) {
        ...
        const newNotes = user.notes.map(item => {
...
export const addNewNoteToVideoHandler = function (schema, request) {
    ...
    if (user) {
        ...
        if (user.notes.some(item => item.videoId === videoId)) {
            const newNotes = user.notes.map(item => {
                if (item.videoId === videoId)
            ...
            this.db.users.update({ notes: newNotes });
            return new Response(201, {}, { notes: newNotes });
        }
        user.notes.push({ videoId, notes: [{ ...note, id: uuid(), createdAt: formatDate() }] });
